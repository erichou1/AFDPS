"""DPS port for the linear inverse-scattering problem.

Paper: Diffusion Posterior Sampling for General Noisy Inverse Problems
Reference: navier_stokes/algo/dps.py (verbatim logic, adapted imports)

DPS backpropagates ∇_x ||y − A(x̂₀)||² through the denoiser at every step.
No SVD required — uses only forward_op.gradient() (autograd through the forward
map).  Requires grad tracking through the network, so more memory than AFDPS.
"""
import torch
